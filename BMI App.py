...
if halaman == "Kalkulator BMI":
    ...
    with col1:
        berat_badan = st.number_input("Berat Badan (kg)", min_value=0.0, value=70.0, step=0.1)
        gender = st.selectbox("Jenis Kelamin", options=["Laki-laki", "Perempuan"])
    with col2:
        tinggi_badan = st.number_input("Tinggi Badan (cm)", min_value=0.0, value=170.0, step=0.1)
...
# ===========================
    ...
    col1, col2 = st.columns(2)
    with col1:
        berat_badan_input = st.number_input("Berat (kg)", value=70.0, step=0.1)
        tinggi_badan_input = st.number_input("Tinggi (cm)", value=170.0, step=0.1)
        usia = st.number_input("Usia (tahun)", value=30, step=1)
    ...
    with col2:
        ...
        lingkar_pinggang = st.number_input("Lingkar Pinggang (cm)", value=85.0, step=0.1)
        lingkar_leher = st.number_input("Lingkar Leher (cm)", value=38.0, step=0.1)
        lingkar_pinggul = None
    
    if gender_input == "Perempuan":
        lingkar_pinggul = st.number_input("Lingkar Pinggul (cm)", value=95.0, step=0.1)
...
with st.sidebar.expander("⚙️ Pengaturan Input Default"):
    default_berat = st.number_input("Berat default (kg)", min_value=0.0, value=70.0, key="default_berat_input")
    default_tinggi = st.number_input("Tinggi default (cm)", min_value=0.0, value=170.0, key="default_tinggi_input")
